window.onload = function() {
	...
	// 商品颜色切换
	{
		document.querySelectorAll('span.color').forEach(function(item) {
			item.onclick = function() {
				...
				this.classList.add('active');
				document.querySelector('.avatar').src = '../images/detail-2-color-' + this.dataset.id + '.jpg';
			};
		});
	}
	...
	// 动态渲染背景图片
	{
		...
		for(let i = 0; i < dts.length; ++i) {
			dts[i].style.backgroundImage = 'url(../images/detail-2-' + (i + 4) + '.jpg)';
		}
		let dtsShort = document.querySelectorAll('.content-item.short dt');
		let images = [
			'url(../images/detail-2-3.jpg)',
