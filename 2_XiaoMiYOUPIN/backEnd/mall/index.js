...
app.use(async (ctx, next) => {
    return next()
        .then((data = null) => {
            console.log('🚀 ~ ctx.state.jwtPayload:', ctx.state.jwtPayload);
            // token验证通过（更新token,重新计算token过期时间）
            if (ctx.state.jwtPayload) {
                ctx.res.setHeader(
                    'Authorization',
                    tokenHelper.generate({ name: ctx.state.jwtPayload.name })
                );
            }
        ...
        .catch(error => {
            ...
            else if (typeof error !== 'string') ctx.body = httpResult.error(null, error.message);
            else if (error === '404') ctx.body = httpResult.notFound();
            else ctx.body = httpResult.failure(null, error);
        });
});
// token验证，分类、商品、及用户相关的操作无需登录直接放行
app.use(
    jwt({ secret: jwtConfig.secret, key: 'jwtPayload' }).unless({
        path: [/^\/category/, /^\/product/, /^\/user/],
    })
);
// 自定义动态路由数据操作请求
