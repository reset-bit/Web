...
router.post('/list', async (ctx, next) => {
    ...
    const sql =
        'select T1.id,T1.pid,T1.count,T2.name,T2.avatar,T2.price,T2.brief from (select * from `dt_cart` where `name` = ?) T1 inner join `dt_product` T2 on T1.pid = T2.id;';
...
router.post('/list_ids', async (ctx, next) => {
    ...
    var sql =
        'select T1.id,T1.pid,T1.count,T2.name,T2.avatar,T2.price,T2.brief from (select * from `dt_cart` where `id` in (?)) T1 inner join `dt_product` T2 on T1.pid = T2.id;';
...
router.post('/remove', async (ctx, next) => {
    ...
    const results = await query(sql, [ids]);
    if (results.affectedRows === ids.length) return Promise.resolve();
    else return Promise.reject('从购物车中删除商品失败..');
});
// 增加数量
router.post('/increase/:id', async (ctx, next) => {
    ...
    else return Promise.reject('数量增加失败..');
});
// 减少数量
