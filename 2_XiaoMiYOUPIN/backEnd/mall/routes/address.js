...
router.post('/add', async (ctx, next) => {
    ...
    const sql =
        'insert `dt_address`(`name`,`receiveName`,`receivePhone`,`receiveRegion`,`receiveDetail`) values(?,?,?,?,?);';
...
router.post('/update', async (ctx, next) => {
    ...
    let sql = 'select * from `dt_address` where `id` = ?;';
    let results = await query(sql, [id]);
    if (
        results[0].receiveName === receiveName &&
        results[0].receivePhone === receivePhone &&
        results[0].receiveRegion === receiveRegion &&
        results[0].receiveDetail === receiveDetail
    )
        return Promise.resolve();
    sql =
        'update `dt_address` set `receiveName`=?,`receivePhone`=?,`receiveRegion`=?,`receiveDetail`=? where `id`=?;';
    results = await query(sql, [receiveName, receivePhone, receiveRegion, receiveDetail, id]);
