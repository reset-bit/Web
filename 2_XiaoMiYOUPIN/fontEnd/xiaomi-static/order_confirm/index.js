...
let pid = 0,
    count = 0,
    isone = true; // 加入购物车需要的数据（从商品详情页跳转需要先加入购物车）
// 动态渲染
{
    ...
    // 判断来源
    if (document.referrer.indexOf('/detail/index.html') !== -1) {
        // 从商品详情页跳转
        pid = $.query.get('pid') || 1;
    ...
    } else if (document.referrer.indexOf('/cart/index.html') !== -1) {
        // 从购物车跳转
        ids = ($.query.get('ids').length
            ? $.query.get('ids').split(',')
            : [$.query.get('ids')]) || [1];
    ...
    $('body').on('click', 'i.checkbox', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                res.data.forEach(function (item) {
                    if (
                        item.receiveRegion.indexOf(region) !== -1 &&
                        item.receiveDetail.indexOf(detail) !== -1
                    ) {
                        addressId = item.id;
