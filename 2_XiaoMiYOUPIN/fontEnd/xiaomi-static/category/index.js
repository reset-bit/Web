...
$('ul.list-main').on('click', function (e) {
    ...
    $.ajax({
        type: 'get',
        url: 'http://localhost:3000/category/list/' + $li.attr('data-id'),
        success: function (res) {
            if (res.code !== 200) {
                console.log(res.msg);
                return;
            ...
            if (res.data.length) {
                res.data.forEach(function (item) {
                    htmlStr += `
						<li data-id=${item.id}>
							<a href="../list/index.html?cid=${item.id}&cName=${item.name}&pid=${$li.attr(
                        'data-id'
                    )}"><img src="../${item.avatar}"><span>${item.name}</span></a>
						</li>
					`;
                });
