							...
							<li data-id="${item.id}" data-pid=${item.pid}>
								...
								<div class="list-content-wrapper">
									...
									<div class="list-content">
										...
										<span class="count-wrapper"><span class="btn-decrease ${
                                            item.count === 1 ? '' : 'active'
                                        }">-</span><span class="count">${
                            item.count
                        }</span><span class="btn-increase active">+</span><span>
										</div>
									</div>
							</div>
							</li>
...
// 点击事件
{
    ...
    $('.content')
        ...
        .on('click', 'i.checkbox', function () {
            $(this).toggleClass('checked');
            $('i.checkbox.all').toggleClass(
                'checked',
                $('i.checkbox:not(.all,.checked)').length === 0
            );
            updateTotal();
        })
        .on('click', 'span.btn-decrease', function () {
            ...
            $.ajax({
                type: 'post',
                headers: { Authorization: Cookies.get('token') },
                url: 'http://localhost:3000/cart/decrease/' + $(this).parents('li').attr('data-id'),
                success: function (res) {
                    if (res.code !== 200) {
                        console.log(res.msg);
                        return;
        ...
        .on('click', 'span.btn-increase', function () {
            ...
            $.ajax({
                type: 'post',
                headers: { Authorization: Cookies.get('token') },
                url: 'http://localhost:3000/cart/increase/' + $(this).parents('li').attr('data-id'),
                success: function (res) {
                    if (res.code !== 200) {
                        console.log(res.msg);
                        return;
