...
function getHtmlStr(item) {
    return `
		<li data-id=${item.id} class="${item.isDefault ? 'default' : ''}">
			<p>
				<span class="receive-name">${item.receiveName}</span><span class="receive-phone">${
        item.receivePhone
    }</span>
			</p>
			<p>
				<span class="is-default">默认</span><span class="receive-region">${
                    item.receiveRegion
                }</span> <span class="receive-detail">${item.receiveDetail}</span>
			</p>
...

{
    ...
    $('ul.list').on('click', 'i.icon-edit', function () {
        ...
        $.ajax({
            type: 'get',
            headers: { Authorization: Cookies.get('token') },
            url: 'http://localhost:3000/address/model/' + $(that).parent().attr('data-id'),
            success: function (res) {
                if (res.code !== 200) {
                    console.log(res.msg);
                    return;
    ...
    $('span.btn-remove').on('click', function () {
        ...
        $.ajax({
            type: 'get',
            headers: { Authorization: Cookies.get('token') },
            url: 'http://localhost:3000/address/remove/' + id,
            success: function (res) {
                if (res.code !== 200) {
                    console.log(res.msg);
                    return;
    ...
    $('.btn-save').on('click', function () {
        ...
        if (id === 0) {
            ...
            if (
                ...
                $.ajax({
                    type: 'post',
                    headers: { Authorization: Cookies.get('token') },
                    url: 'http://localhost:3000/address/add',
            ...
            $.ajax({
                type: 'post',
                headers: { Authorization: Cookies.get('token') },
                url: 'http://localhost:3000/address/update',
        ...
        if (isDefault) {
            ...
            $.ajax({
                type: 'get',
                headers: { Authorization: Cookies.get('token') },
                url: 'http://localhost:3000/address/get_default',
                success: function (res) {
                    if (res.code !== 200) {
                        console.log(res.msg);
                        return;
            ...
            $.ajax({
                type: 'get',
                headers: { Authorization: Cookies.get('token') },
                url: 'http://localhost:3000/address/set_default/' + id,
                success: function (res) {
                    if (res.code !== 200) {
                        console.log(res.msg);
                        return;
