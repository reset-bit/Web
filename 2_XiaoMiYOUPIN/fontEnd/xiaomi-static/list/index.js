...
// 商品列表动态渲染、加载更多、排序切换
{
    ...
    $.ajax({
        type: 'get',
        url: 'http://localhost:3000/category/list/' + pid,
    ...
    $('.order-wrapper').on('click', 'span', function (e) {
        if ($(this).hasClass('active') && $(this).attr('data-col') === 'price') {
            // 针对价格切换升降序
            orderDir = $(this).hasClass('asc') ? 'desc' : 'asc';
            $(this)
                .toggleClass('asc desc')
                .children('i')
                .toggleClass('icon-sort_asc icon-sort_desc');
        } else {
            // 切换排序方式
...
// 搜索模态框
{
    ...
    setInterval(function () {
        ...
        $(`ul.search-list>li:nth-child(${index + 1})`)
            .addClass('active')
            .siblings('.active')
            .removeClass('active');
        setTimeout(function () {
